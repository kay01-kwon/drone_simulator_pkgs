...
def generate_launch_description():
    ...
    # --- ros_gz_bridge (PX4 mode: no motor command bridge) ---
    bridge = Node(
    ...
    # --- PX4 SITL ---
    # PX4 gz_bridge connects directly to Gazebo via gz-transport
    # Environment variables tell PX4 to use our S550 model
    px4_sitl = ExecuteProcess(
        cmd=[
            'bash', '-c',
            ' '.join([
                'cd', LaunchConfiguration('px4_dir'), '&&',
                'PX4_SYS_AUTOSTART=' + LaunchConfiguration('px4_autostart').describe(),
